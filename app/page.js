...
import { collection, query, getDocs, setDoc, doc, updateDoc, deleteDoc, getDoc } from 'firebase/firestore'
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // All Firebase-related code should be inside this check
      updateInventory();
    }
  }, [])
  ...
  return (
    <Box width="100vw" height="100vh" display="flex" flexDirection="column" justifyContent="center" alignItems="center" gap={2}>
      <Modal open={open} onClose={handleClose}>
        <Box position="absolute" top="50%" left="50%" transform="translate(-50%, -50%)" width={400} bgcolor="white" border="2px solid #000" boxShadow={24} p={4} display="flex" flexDirection="column" gap={3} sx={{transform: 'translate(-50%, -50%)'}}>
...
export default function Home() {
  ...
  return (
  <Box width="100vs" height="100vh" display="flex" flexDirection="column" justifyContent="center" alignItems="center" gap={2}>
    <Modal open={open} onClose={handleClose}>
      <Box position="absolute" top="50%" left="50%" transform="translate(-50%, -50%)" width={400} bgcolor="white" border="2px solid #000" boxShadow={24} p={4} display="flex" flexDirection="column" gap={3} sx={{transform: 'translate(-50%, -50%)'}}>
        <Typography variant="h6" component="h2">Add Item</Typography>
        <Stack direction="row" width="100%" spacing={3}>
